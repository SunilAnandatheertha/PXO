function [GrainSizes_mtex, numNeighbors_mtex, equivalentRadius_mtex, equivalentPerimeter_mtex, ShapeFactor_mtex] = Calculate_GS_Data_using_mtex()
...
for rset = 1:N_Time_Slices % Time slice level
    disp('---------------------------------------------------------')
    fprintf(['Working on MC time slice # ' num2str(TimeSteps(rset)) '\n'])
    %---------------------------------------------------------
...
% %     % and plot the on top
% %     hold on
% %     plot(tP,'color','b','linewidth',1, 'markersize', 4)
