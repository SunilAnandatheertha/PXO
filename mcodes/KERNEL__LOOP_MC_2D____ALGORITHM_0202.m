function KERNEL__LOOP_MC_2D____ALGORITHM_0202(initialmcs, finalmcs, consider_energy, E, delE, ham, SZS1P1, SZS1P2, SZS1P3, vf, skipmatrix, ConsoleDisplay)
%-------------------------------------------------------------
...
q_trans_prob_A = 100.0;
q_trans_prob_B = 100.0;
q_trans_prob = exp(-1*(q_trans_prob_A + q_trans_prob_B*rand(1,q)));
% q_trans_prob = round(exp(-1*(q_trans_prob_a + q_trans_prob_b*rand(1,q))))*rand(1);
%-------------------------------------------------------------
for ms = start:finalmcs
    for pt = 1:numel(s)
        if skipmatrix(pt) == 0
            ...
            if (energy2-energy1)<=0 || rand(1)<=q_trans_prob(esm5)
                s(pt)   = NS;
