...
function [zenerindices, zslsp,zslspc, cntele,zcnt, cntxythetainfo] = vasudeva3d(latpar,x,y,z, e,slspinfo, icntinfo, clusterinfo, runfuncvasudeva)
...
%                 plot(x(zslspc{clust,1}),y(zslspc{clust,1}),'r.')
            ...
            % following 'for' loop plots the slspc
            for count = 1:size(zslspc,1)
                plot3(x(zslspc{count}),y(zslspc{count}),z(zslspc{count}),'k.'),hold on
            end
            % above 'for' loop plotted the slspc
            zslspctemp2 = [];
            for count = 1:size(zslspc,1)
...
%% SUB-FUNCTION: createCNTs2d

function [cntele,cntxythetainfo] = createCNTs2d(latpar,icntinfo)
