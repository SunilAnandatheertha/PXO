...
%                                PlotAtoms,PlotCovEle,PlotVd1Ele,PlotVd2Ele,PlotDifferentViewsInSP,StoredStackNumber)
function GRX_PlotGrapheneStack(xgstack,    ygstack,                zgstack,          cgstack,...
                               vd1gstack,  vd2gstack,              vd3gstack,        GrapheneStackLayUp,...
                               PlotAtoms,  PlotCovEle,             PlotVd1Ele,       PlotVd2Ele,...
                               PlotVd3Ele, PlotDifferentViewsInSP, StoredStackNumber, varargin)
...
% FIGURE01_h = figure('renderer','opengl');
FIGURE01_h = figure;
hold on
...
for ct1 = 1:NoOfStacks
    ...
    if PlotCovEle == 1
        for ct2 = 1:size(covalent,1)
            % USE LINE COMMANDS INSTEAD OF PLOT AS THEY SPEED BY ABOUT 25.00 %
%             plot3([covalent(ct2,3) covalent(ct2,5)],...
%                 [covalent(ct2,4) covalent(ct2,6)],...
%                 [max(max(zgstack{1,ct1})) max(max(zgstack{1,ct1}))],...
%                 'k','LineWidth',1)
            line([covalent(ct2,3) covalent(ct2,5)],...
                [covalent(ct2,4) covalent(ct2,6)],...
                [max(max(zgstack{1,ct1})) max(max(zgstack{1,ct1}))],'LineStyle','-','Color','k', 'LineWidth',2)
        end
    end
...
%                 [max(max(zgstack{1,ct1})) max(max(zgstack{1,ct1}))],...
%                 'k--','LineWidth',0.1)
            line([vanderwall1(ct2,3) vanderwall1(ct2,5)],...
                [vanderwall1(ct2,4) vanderwall1(ct2,6)],...
                [max(max(zgstack{1,ct1})) max(max(zgstack{1,ct1}))],'LineStyle','-.','Color','b')
        end
    end
...
%                   [max(max(zgstack{1,ct1})) max(max(zgstack{1,ct1}))],...
%                   'k:', 'LineWidth', 0.1)
            line([vanderwall2(ct2,3) vanderwall2(ct2,5)],...
                  [vanderwall2(ct2,4) vanderwall2(ct2,6)],...
                  [max(max(zgstack{1,ct1})) max(max(zgstack{1,ct1}))],'LineStyle','--','Color','b')
        end
    end
    %%%%%%%% PLOT VAN-DER WALLS 3 BOND FINITE ELEMENTS
    if ct1 > 1
        if PlotVd3Ele == 1
            vanderwall3 = vd3gstack{1, ct1-1};
            for ct2 = 1:size(vanderwall3, 1)
%                 plot3([vanderwall3(ct2,3) vanderwall3(ct2,6)],...
%                       [vanderwall3(ct2,4) vanderwall3(ct2,7)],...
%                       [vanderwall3(ct2,5) vanderwall3(ct2,8)],...
%                       'r:', 'LineWidth', 0.1)
                line([vanderwall3(ct2,3) vanderwall3(ct2,6)],...
                      [vanderwall3(ct2,4) vanderwall3(ct2,7)],...
                      [vanderwall3(ct2,5) vanderwall3(ct2,8)],'LineStyle',':','Color','r')
            end
        end
    end
end
...
if PlotDifferentViewsInSP == 1
    ...
    axis equal, axis tight, box on, xlabel('x-axis (WIDTH), nm'), ylabel('y-axis (LENGTH), nm'), zlabel('z-axis (THICKNESS), nm')
