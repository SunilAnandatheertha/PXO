...
# Summary
Poly-Xtal operations ('PXO') is a MATLAB based computational platform enabling users to create and prepare realistic grain structures for finite element analysis, simulate complex grain growth phenomenon and achieve spatiotemporal partition a 2D space into a set of closed regions. In addition to various in-built tools, 'PXO' can output the grain structure in a format which can be further imported into 3rd party open source libraries to tend to the specific requirements of the user. It allows easy inclusion of user defined physics based models for grain growth. Monte-Carlo simulations (MCS) form the heart of PXO. However, non-temporal grain structures instances of the Voronoi tessellation (VT) types can also be generated over different types of underlying lattices. Equivalent VT grain structures of MCS based temporal slices of grain structures.

# Documentations
All documentations, tutprials and test cases can be accessed from [PXO/Wiki](https://github.com/SunilAnandatheertha/PXO/wiki).
