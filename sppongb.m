...
%% PHI vs PSI -- SLSP
figure(2),hold on
%vf slsp = 0.0020 i.e. 0.2%
plot(1*[500^2].^(-1),[0.6520],...
    'hk','LineWidth',1,'MarkerSize',7)
%vf slsp = 0.0040 i.e, 0.4%
plot(1*[500^2].^(-1),[0.723],...
    'dk','LineWidth',1,'MarkerSize',7)
%vf slsp = 0.0080
plot(1*[200^2 300^2 500^2].^(-1),[(320-(080+092+76)/3)/320 (720-(198)/1)/720 0.814],...
    'sk','LineWidth',1,'MarkerSize',7)
%vf slsp = 0.0120
plot(1*[200^2 500^2].^(-1),[(480-(111+080)/2)/480 0.8513],...
    'ok','LineWidth',1,'MarkerSize',7)
%vf slsp = 0.0150
plot(1*[200^2].^(-1),...
    [(totalZPav_200sq-nslspgb_mcs_av_200sq(size(nslspgb_mcs_av_200sq,1),1))/totalZPav_200sq],...
    '<k','LineWidth',1,'MarkerSize',7)
%vf slsp = 0.0175
plot(1*[200^2 300^2].^(-1),[(700-(158+128)/2)/700 0.8336],...
    '^k','LineWidth',1,'MarkerSize',7)
%vf slsp = 0.0200
plot(1*[200^2 300^2].^(-1),[(800-(136+162)/2)/800 (1800-(334)/1)/1800],...
    '>k','LineWidth',1,'MarkerSize',7)
legend_h = legend('spp.0.20\%',...
                  'spp.0.40\%',...
                  'spp.0.80\%',...
                  'spp.1.20\%',...
                  'spp.1.50\%',...
                  'spp.1.75\%',...
                  'spp.2.00\%',...
                  'Location','SouthEast');%'spp.0.0762,61,32,1.00',...
